...
public class LinuxStateForShell {
     ...
     * @return 最终处理后的信息
     */
    public static Map<String, String> disposeResultMessage(Map<String, String> result) {
        ...
        for (String command : COMMANDS) {
            ...
            if (command.equals(CPU_MEM_SHELL)) {
                ...
                try {
                    List<String> strs = disposeFilesSystem(commandResult);
     ...
     * @return 处理后的结果
     */
    public static List<String> disposeFilesSystem(String commandResult) {
        String[] strings = commandResult.split(LINE_SEPARATOR);
        // final String PATTERN_TEMPLATE = "([a-zA-Z0-9%_/]*)\\s";
        List<String> diskInfos = new ArrayList<String>();
        for (int i = 0; i <= strings.length - 1; i++) {
            if (i == 0) continue;
//            diskInfos.
            diskInfos.add(strings[i]);
        }
        return diskInfos;
    }

    public static void main(String[] args) {
        Map<String, String> result = runDistanceShell(COMMANDS, "deploy", "DEPLOY_sanss123", "172.16.53.131");
